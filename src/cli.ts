...
import { CompletionConfig, ShellType } from './types/index.js';
  ...
  .action(async (command: string, options) => {
    ...
    try {
      ...
      if (options.allShells) {
        ...
        const shellType = options.shell as ShellType;
      ...
      logger.error('CLI command failed', error as Error);
...
async function writeCompletionFile(content: string, filePath: string): Promise<void> {
...
async function installCompletion(script: any, command: string): Promise<void> {
