...
export class CompletionGeneratorTool extends BaseTool {
    constructor() {
        super('completion-generator', 'Generate shell completion scripts from command documentation');
    }
    validateArgs(args) {
        try {
    ...
    async execute(args, context) {
        ...
        try {
            ...
            throw new CompletionGenerationError(`Failed to generate completion script: ${error.message}`, error);
        }
    }
    ...
    generateCompletionScript(commandInfo, shellType, enableAdvancedFeatures) {
        switch (shellType) {
            case 'bash':
    ...
    generateBashCompletion(commandInfo, enableAdvancedFeatures) {
...
_${cmd}_completion() {
    ...
    case "\${prev}" in
        ${commandInfo.options.filter(opt => opt.hasValue).map(opt => `${opt.flag})\n            COMPREPLY=($(compgen -f -- "\${cur}"))\n            return 0\n            ;;`).join('\n        ')}
    esac
...
complete -F _${cmd}_completion ${cmd}`;
    ...
    generateZshCompletion(commandInfo, enableAdvancedFeatures) {
        const cmd = commandInfo.command;
        const options = commandInfo.options.map(opt => {
            const hasValue = opt.hasValue ? ':file:_files' : '';
            return `  '${opt.flag}[${opt.description}]${hasValue}'`;
        }).join('\n');
        const subcommands = commandInfo.subcommands.map(sub => `    '${sub}:${sub} command'`).join('\n');
        return `#compdef ${cmd}
...
_${cmd} "$@"`;
    ...
    generateFishCompletion(commandInfo, enableAdvancedFeatures) {
        const cmd = commandInfo.command;
        const options = commandInfo.options.map(opt => {
        ...
        const subcommands = commandInfo.subcommands.map(sub => `complete -c ${cmd} -n '__fish_use_subcommand' -a '${sub}' -d '${sub} command'`).join('\n');
...
complete -c ${cmd} -f`;
    ...
    generateInstallInstructions(command, shellType) {
