...
export class HelpExtractorTool extends BaseAsyncTool {
    constructor() {
        super('help-extractor', 'Extract help text from CLI commands using various help flags', 10000);
    }
    validateArgs(args) {
        try {
    ...
    async execute(args, context) {
        ...
        for (const flag of helpFlags) {
            try {
                const result = await this.executeWithTimeout(() => this.extractHelpText(command, flag), timeout);
                results.push(result);
        ...
        if (!bestResult || (!bestResult.helpText.trim() && bestResult.exitCode !== 0)) {
            throw new CommandExecutionError(`Failed to extract help text for command: ${command}. Tried flags: ${helpFlags.join(', ')}`);
        }
        return {
            success: true,
            data: bestResult,
    ...
    async extractHelpText(command, flag) {
